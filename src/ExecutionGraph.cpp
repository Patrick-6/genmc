...
bool ExecutionGraph::isMaximalEvent(const EventLabel *lab, const WriteLabel *wLab)
{
...
bool ExecutionGraph::inMaximalPath(const ReadLabel *rLab, const WriteLabel *wLab)
{
	...
	if (!coherenceSuccRemainInGraph(*this, rLab, llvm::dyn_cast<WriteLabel>(wLab), v))
		return false;

	// llvm::dbgs() << "checking intermediates\n";
	for (auto i = 0u; i < getNumThreads(); i++) {
		for (auto j = getThreadSize(i) - 1; j != 0u; j--) {
			auto *lab = getEventLabel(Event(i, j));
			...
			if (auto *rLabB = llvm::dyn_cast<ReadLabel>(lab)) {
				if (getEventLabel(rLabB->getRf())->getStamp() > rLabB->getStamp() &&
				    !v.contains(rLabB->getRf()) && !rLabB->isRevisitedInPlace()) {
					// llvm::dbgs() << "RF WILL BE DELETED\n";
					return false;
				}
				if (!readsFromMaximalInRevGraph(*this, rLabB, rLab, v, llvm::dyn_cast<WriteLabel>(wLab), initMaximals))
					return false;
			}
