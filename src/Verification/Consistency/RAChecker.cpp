...
const View &RAChecker::getHbView(const EventLabel *lab) const { return lab->view(0); }
...
std::vector<Event> RAChecker::getInitRfsAtLoc(const ExecutionGraph &g, SAddr addr)
{
...
ExecutionGraph::const_co_iterator RAChecker::splitLocMOBefore(const ExecutionGraph &g, SAddr addr,
							      Event e)
{
	auto rit = std::find_if(g.co_rbegin(addr), g.co_rend(addr),
				[&](auto &lab) { return isWriteRfBefore(g, lab.getPos(), e); });
...
ExecutionGraph::const_co_iterator RAChecker::splitLocMOAfterHb(const ExecutionGraph &g, SAddr addr,
							       const Event read)
{
	...
	auto it = std::find_if(g.co_begin(addr), g.co_end(addr),
			       [&](auto &lab) { return isHbOptRfBefore(g, read, lab.getPos()); });
	if (it == g.co_end(addr) || it->view(0).contains(read))
		return it;
	return ++it;
}

ExecutionGraph::const_co_iterator RAChecker::splitLocMOAfter(const ExecutionGraph &g, SAddr addr,
							     const Event e)
{
	return std::find_if(g.co_begin(addr), g.co_end(addr),
			    [&](auto &lab) { return isHbOptRfBefore(g, e, lab.getPos()); });
}

std::vector<Event> RAChecker::getCoherentStores(const ExecutionGraph &g, SAddr addr, Event read)
{
...
std::vector<Event> RAChecker::getMOOptRfAfter(const ExecutionGraph &g, const WriteLabel *sLab)
{
...
std::vector<Event> RAChecker::getMOInvOptRfAfter(const ExecutionGraph &g, const WriteLabel *sLab)
{
	std::vector<Event> after;
	std::vector<const ReadLabel *> rfAfter;
...
static std::vector<Event> getRevisitableFrom(const ExecutionGraph &g, const WriteLabel *sLab,
...
std::vector<Event> RAChecker::getCoherentRevisits(const ExecutionGraph &g, const WriteLabel *sLab,
...
std::vector<Event> RAChecker::getCoherentPlacings(const ExecutionGraph &g, SAddr addr, Event store,
						  bool isRMW)
{
	std::vector<Event> result;
...
bool RAChecker::visitCoherence_1(const EventLabel *lab) const
{
	...
	visitedCoherence_1[lab->getStamp().get()] = {visitedCoherenceAccepting,
						     NodeStatus::entered};
		...
		if (auto pLab = po_imm_pred(g, lab); pLab) {
			...
			if (node.status == NodeStatus::unseen) {
				if (!visitCoherence_5(pLab)) {
				   ...
				   (visitedCoherenceAccepting > node.count || 0)) {
	...
	if (auto pLab = po_imm_pred(g, lab); pLab) {
		...
		if (node.status == NodeStatus::unseen) {
			if (!visitCoherence_1(pLab)) {
			   ...
			   (visitedCoherenceAccepting > node.count || 1)) {
...
bool RAChecker::visitCoherence_2(const EventLabel *lab) const
{
	...
	visitedCoherence_2[lab->getStamp().get()] = {visitedCoherenceAccepting,
						     NodeStatus::entered};
...
bool RAChecker::visitCoherence_3(const EventLabel *lab) const
{
	...
	visitedCoherence_3[lab->getStamp().get()] = {visitedCoherenceAccepting,
						     NodeStatus::entered};
...
bool RAChecker::visitCoherence_4(const EventLabel *lab) const
{
	...
	visitedCoherence_4[lab->getStamp().get()] = {visitedCoherenceAccepting,
						     NodeStatus::entered};
...
bool RAChecker::visitCoherence_5(const EventLabel *lab) const
{
	...
	visitedCoherence_5[lab->getStamp().get()] = {visitedCoherenceAccepting,
						     NodeStatus::entered};
...
bool RAChecker::visitCoherenceFull(const ExecutionGraph &g) const
{
	...
	return true && std::ranges::all_of(g.labels(), [&](auto &lab) {
		       return visitedCoherence_1[lab.getStamp().get()].status !=
				      NodeStatus::unseen ||
			      visitCoherence_1(&lab);
	       });
}
