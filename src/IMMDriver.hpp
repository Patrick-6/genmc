...
private:
	...
	DepView getDepsAsView(const DepInfo *deps);
	View calcBasicHbView(Event e) const;
	DepView calcPPoView(Event e, const DepInfo *deps); /* not const */
	...
	void calcLockLAPORViews(LockLabelLAPOR *lab, const DepInfo *deps);
};
