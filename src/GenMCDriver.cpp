...
Event findNextLabelToAdd(const ExecutionGraph &g, Event pos)
{
	...
	auto it = std::find_if(po_succ_begin(g, first), po_succ_end(g, first),
			    [&](auto &e){ return llvm::isa<EmptyLabel>(g.getEventLabel(e)); });
	return it == po_succ_end(g, first) ? g.getLastThreadEvent(pos.thread).next() : *it;
}
...
SVal GenMCDriver::getWriteValue(const EventLabel *lab, SAddr addr, AAccess access)
{
...
SVal GenMCDriver::getDskWriteValue(const EventLabel *lab, SAddr addr, AAccess access)
{
	if (lab->getPos().isInitializer())
		return SVal();
	return getWriteValue(lab, addr, access);
}
...
SVal GenMCDriver::getRecReadRetValue(const ReadLabel *rLab)
{
	...
	return getWriteValue(g.getEventLabel(rf), rLab->getAddr(), rLab->getAccess());
}
...
void GenMCDriver::checkLockValidity(const ReadLabel *rLab, const std::vector<Event> &rfs)
{
	...
	auto rfIt = std::find_if(rfs.cbegin(), rfs.cend(), [this, lLab](const Event &rf){
		auto rfVal = getWriteValue(getGraph().getEventLabel(rf), lLab->getAddr(), lLab->getAccess());
		return rfVal == SVal(-1);
	});
	if (rfIt != rfs.cend())
...
void GenMCDriver::checkBIncValidity(const ReadLabel *rLab, const std::vector<Event> &rfs)
{
	...
	if (std::any_of(rfs.cbegin(), rfs.cend(), [](const Event &rf){ return rf.isInitializer(); }))
		...
		auto rfVal = getWriteValue(getGraph().getEventLabel(rf), bLab->getAddr(), bLab->getAccess());
		return rfVal == SVal(0);
	}))
...
bool GenMCDriver::filterValuesFromAnnotSAVER(const ReadLabel *rLab, std::vector<Event> &validStores)
{
	...
	validStores.erase(std::remove_if(validStores.begin(), validStores.end(), [&](Event w){
		auto val = getWriteValue(g.getEventLabel(w), rLab->getAddr(), rLab->getAccess());
	...
	auto maximalVal = getWriteValue(g.getEventLabel(validStores.back()), rLab->getAddr(), rLab->getAccess());
...
std::vector<Event> GenMCDriver::getRfsApproximation(const ReadLabel *lab)
{
	...
	rfs.erase(std::remove_if(rfs.begin(), rfs.end(), [&](const Event &s){
		auto oldVal = getWriteValue(g.getEventLabel(s), lab->getAddr(), lab->getAccess());
...
void GenMCDriver::filterConfirmingRfs(const ReadLabel *lab, std::vector<Event> &stores)
{
	...
	auto valid = std::count_if(stores.begin(), stores.end(), [&](const Event &s){
		return getWriteValue(g.getEventLabel(s), rLab->getAddr(), rLab->getAccess()) == specVal;
	});
...
GenMCDriver::handleLoad(std::unique_ptr<ReadLabel> rLab)
{
	...
	auto retVal = getWriteValue(g.getEventLabel(stores.back()), lab->getAddr(), lab->getAccess());
...
std::unique_ptr<VectorClock>
GenMCDriver::getPrefixView(Event e)
{
	auto &g = getGraph();
	// if (!getConf()->isDepTrackingModel)
	// 	return  std::make_unique<View>(g.getEventLabel(e)->getPorfView());
	// return std::make_unique<DepView>(g.getEventLabel(e)->getPPoRfView());
	ERROR("Unimplemented view");
}
...
GenMCDriver::getRevisitView(const BackwardRevisit &r) const
{
	...
	updatePredsWithPrefixView(g, *preds, *r.getPrefixNoRel());
	if (auto *br = llvm::dyn_cast<BackwardRevisitHELPER>(&r))
		updatePredsWithPrefixView(g, *preds, *br->getMidPrefixNoRel());
...
{
	if (!getConf()->helper)
		return LLVM_MAKE_UNIQUE<BackwardRevisit>(rLab, sLab, getPrefixView(sLab->getPos()));
	...
	if (!pending.isInitializer() &&
		...
		return LLVM_MAKE_UNIQUE<BackwardRevisitHELPER>(rLab->getPos(), sLab->getPos(), std::move(prefix), pending, getPrefixView(pending));
	return LLVM_MAKE_UNIQUE<BackwardRevisit>(rLab, sLab, std::move(prefix));
}
...
bool GenMCDriver::prefixContainsMatchingLock(const BackwardRevisit &r, const EventLabel *lab)
{
	...
	if (l.isInitializer())
		return false;
	if (r.getPrefixNoRel()->contains(l))
		return true;
	if (auto *br = llvm::dyn_cast<BackwardRevisitHELPER>(&r))
		return br->getMidPrefixNoRel()->contains(l);
	return false;
}
					...
					const EventLabel *lab) const
{
	...
	auto &g = getGraph();
	auto &v = *llvm::dyn_cast<DepView>(&*r.getPrefixNoRel());
	if (lab->getIndex() <= v.getMax(lab->getThread()) && isFixedHoleInView(g, lab, v))
		return true;
	if (auto *br = llvm::dyn_cast<BackwardRevisitHELPER>(&r)) {
		auto &hv = *llvm::dyn_cast<DepView>(&*br->getMidPrefixNoRel());
...
SVal GenMCDriver::handleDskRead(std::unique_ptr<DskReadLabel> drLab)
{
	...
	return getDskWriteValue(g.getEventLabel(validStores[0]), lab->getAddr(), lab->getAccess());
}
