...
View SCDriver::calcBasicPorfView(Event e) const
{
...
void SCDriver::calcBasicViews(EventLabel *lab)
{
...
void SCDriver::calcReadViews(ReadLabel *lab)
{
	const auto &g = getGraph();
	View hb = calcBasicHbView(lab->getPos());
	View porf = calcBasicPorfView(lab->getPos());
...
void SCDriver::calcWriteViews(WriteLabel *lab)
{
	calcBasicViews(lab);
...
void SCDriver::calcWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void SCDriver::calcRMWWriteMsgView(WriteLabel *lab)
{
	const auto &g = getGraph();
	View msg;
...
void SCDriver::calcFenceRelRfPoBefore(Event last, View &v)
{
	const auto &g = getGraph();
...
void SCDriver::calcFenceViews(FenceLabel *lab)
{
	const auto &g = getGraph();
	View hb = calcBasicHbView(lab->getPos());
	View porf = calcBasicPorfView(lab->getPos());
...
void SCDriver::calcStartViews(ThreadStartLabel *lab)
{
	const auto &g = getGraph();
...
void SCDriver::calcJoinViews(ThreadJoinLabel *lab)
{
	const auto &g = getGraph();
...
bool SCDriver::areInDataRace(const MemAccessLabel *aLab, const MemAccessLabel *bLab)
{
...
Event SCDriver::findRaceForNewLoad(const ReadLabel *rLab)
{
	const auto &g = getGraph();
...
Event SCDriver::findRaceForNewStore(const WriteLabel *wLab)
{
	const auto &g = getGraph();
...
Event SCDriver::findDataRaceForMemAccess(const MemAccessLabel *mLab)
{
...
bool SCDriver::visit0(const Event &e)
{
	...
	if (visited0[lab->getStamp()])
		return true;

	visited0[lab->getStamp()] = true;
	for (const auto &s : fr_imm_succs(g, e))
		if (visit0(s)) return true;
...
bool SCDriver::isConsistent(const Event &e)
{
	...
	visited0.clear();
	visited0.resize(lab->getStamp() + 1);
