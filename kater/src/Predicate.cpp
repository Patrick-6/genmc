...
static inline PredicateMask operator~(PredicateMask m)
{
...
std::ostream &operator<<(std::ostream& ostr, const PredicateSet &s)
{
	...
	for (const auto &p : PredicateSet::builtins) {
		...
		if ((mm | s.mask) != mm) continue;
		if ((mm | m) == PredicateMask::False) continue;
		m = m | ~mm;
...
std::string PredicateSet::toGenmcString() const
{
	auto m = mask;
